...
lep.data$Burrow < as.factor(lep.data$DietBurrow)
...
imp <- mice(data_mice, m=5, pred = pred, seed=542, meth = "phpmm", psi = prec$psi, psiinv = prec$psiinv, maxit = 2000)
...
formula1a <- ECV_T ~ SeT * SeP * Burrow * AdultBodyMass
formula2a <- ECV_OB ~ Burrow * Locomotor_Mode * Activity * AdultBodyMass
formula3a <- ECV_ROB ~ Burrow * Locomotor_Mode * Activity * AdultBodyMass

formula1b <- ECV_T ~ DietBreadth * GR_Area * AdultBodyMass
formula2b <- ECV_OB ~ DietBreadth * GR_Area * AdultBodyMass
formula3b <- ECV_ROB ~ DietBreadth * GR_Area * AdultBodyMass

formula1c <- ECV_T ~ GestationLen * LitterSize * AdultBodyMass
formula2c <- ECV_OB ~ GestationLen * LitterSize * AdultBodyMass
formula3c <- ECV_ROB ~ SGestationLen * LitterSize * AdultBodyMass
...
*************************
model.Ran3<-pgls(ECV_ROB ~ AdultBodyMass + HomeRange + Burrow, data = leporid, lambda='ML')
anova(model.Ran3)
*************************

#models eco

model.Act1<-pgls(ECV_OB ~ AdultBodyMass + DietBreadth + Burrow + Activity, data = leporid, lambda='ML')
anova(model.Act1)
